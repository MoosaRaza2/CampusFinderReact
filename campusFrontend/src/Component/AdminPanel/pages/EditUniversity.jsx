...
const EditUniversity = () => {
  ...
  console.log(initialScholarships);
  ...
  const fetchSpecificUniversity = (id) => {
    axios.get(`/getUni/${id}`).then((res) => {
      console.log(res.data);
      var options = { year: 'numeric', month: 'long', day: 'numeric' };

      const [day, month, year] = res.data.data.deadline.split('-');

      const date = `${year}-${month}-${day}`;
      res.data.data.deadline = date;
      console.log(date);
      setUniversityData(res.data.data);
    });
  };
  console.log(universityData?.admissions);
  const postData = async (e) => {
    ...
    const res = await fetch(`/EditUni/${id}`, {
      method: 'POST',
    ...
    axios
      .post(`/addScholarships/${id}`, universityData?.scholarships)
      .then((res) => {
  ...
  return (
    <Grid container>
      ...
      <Grid item container justifyContent='center' mt={1} mb={1}>
        <Header text='Edit University Data' />
      </Grid>
      <Grid
        container
      ...
      >
        ...
        >
          ...
          <Grid item>
            {image && <img src={image} width='400px' height='100px' />}
            {typeof universityData?.logo === 'string' && (
              <img
                src={`/Upload/${universityData?.logo}`}
                width='400px'
                height='100px'
              />
            )}
